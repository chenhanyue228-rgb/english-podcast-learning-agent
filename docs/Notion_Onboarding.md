...
3. 检查数据库编号配置状态。
4. 开始创建前记录安全恢复状态。
5. 每成功创建一个数据库就立即保存该数据库编号。
6. 重试时要求继续使用首次创建数据库时的同一个父页面。
7. 如果父页面不一致或旧设置缺少父页面编号，在任何数据库操作前安全停止。
8. 父页面一致后验证已保存编号，仅继续创建缺失数据库。
9. 来源不明且没有安全恢复标记的部分配置仍会停止，避免重复数据库。
10. 四个编号齐全后重跑数据库关系配置。
11. 关系配置和工作区验证全部成功后才标记设置完成。
